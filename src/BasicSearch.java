...
public class BasicSearch extends HttpServlet {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -3582687437690436688L;
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			String query = "select t1.id, t1.title, genres, year, director, banner_url, trailer_url, fn, ln from "
					+ "(SELECT m.id, title, year, director, banner_url, trailer_url, GROUP_CONCAT(first_name separator ',') as fn, GROUP_CONCAT(last_name separator ',') as ln "
					+ "FROM movies m , stars s, stars_in_movies sm WHERE s.id = sm.star_id AND sm.movie_id = m.id AND m.title like '"
					+ request.getParameter("tag") + "%' GROUP BY m.title, m.id) t1 left join "
					+ "(SELECT m.id, title, group_concat(name separator ',') as genres "
					+ "FROM movies m , genres g, genres_in_movies gim WHERE m.id = gim.movie_id AND gim.genre_id = g.id GROUP BY m.title, m.id) t2 on t1.title=t2.title";
			
			String total = "select COUNT(*) from "
					+ "(SELECT m.id, title, year, director, banner_url, trailer_url, GROUP_CONCAT(first_name separator ',') as fn, GROUP_CONCAT(last_name separator ',') as ln "
					+ "FROM movies m , stars s, stars_in_movies sm WHERE s.id = sm.star_id AND sm.movie_id = m.id AND m.title like '"
					+ request.getParameter("tag") + "%' GROUP BY m.title, m.id) t1 left join "
					+ "(SELECT m.id, title, group_concat(name separator ',') as genres "
		...
		request.setAttribute("type", "BasicSearch");
